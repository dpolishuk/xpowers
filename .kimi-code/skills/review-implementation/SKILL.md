...
description: Use after /skill:executing-plans completes all tasks - verifies implementation against bd spec, all success criteria met, anti-patterns avoided
---

<kimi_compat>
This skill was ported to Kimi Code CLI. In Kimi:
- Load skills with `/skill:name` or let the model invoke them automatically.
- "SetTodoList" means use the `SetTodoList` tool.
- "Agent()" / "Agent" means use the built-in `Agent` tool.
- "parallel Agent calls with run_in_background=true" means dispatch multiple `Agent` calls with `run_in_background=true`.
...
|------|--------|-------------|
| 1 | Load bd epic + all tasks | SetTodoList with tasks to review |
...
<when_to_use>
- /skill:executing-plans completed all tasks
- Before claiming work is complete
- Before /skill:finishing-a-development-branch
- Want to verify implementation matches spec

**Don't use for:**
- Mid-implementation (use /skill:executing-plans)
...
**Announce:** "I'm using /skill:review-implementation to verify implementation matches spec. Reviewing with Google Fellow-level scrutiny."
...
**IMPORTANT:** Use /skill:test-runner agent to avoid context pollution.

```
Dispatch /skill:test-runner: "Run: cargo test"
Dispatch /skill:test-runner: "Run: cargo fmt --check"
Dispatch /skill:test-runner: "Run: cargo clippy -- -D warnings"
Dispatch /skill:test-runner: "Run: .git/hooks/pre-commit"
```
...
- Use /skill:test-runner for tests/lints
...
### J. Mark Task Reviewed (SetTodoList)
...
Ready to proceed to /skill:finishing-a-development-branch.
```
...
**If APPROVED:**
```
Announce: "I'm using /skill:finishing-a-development-branch to complete this work."
...
- [ ] SetTodoList tracker shows all tasks reviewed
...
- /skill:executing-plans (Step 5, after all tasks executed)

**This skill calls:**
- /skill:finishing-a-development-branch (if approved)
- /skill:test-runner agent (for quality gates)

**This skill uses:**
- /skill:verification-before-completion principles (evidence before claims)

**Call chain:**
```
/skill:executing-plans → /skill:review-implementation → /skill:finishing-a-development-branch
                         ↓
                   (if gaps: STOP)
```
